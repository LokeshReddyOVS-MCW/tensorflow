...
absl::Status PrepareImpl(ConvolutionOp& op, const Tensor& lhs,
  ...
  if (!CheckOutputSpacial(op, lhs, rhs, output)) {
    return absl::FailedPreconditionError(
        "stablehlo.convolution constraint violation: "
  ...
  // Transpose prepare
  std::vector<Axis> lhs_permutation_values(lhs.Rank());
  ...
  std::vector<Axis> rhs_permutation_values(static_cast<int64_t>(rhs.Rank()));
  ...
  std::vector<Axis> output_permutation_values(
  ...
  std::vector<Axis> lhs_contracting_dimensions_values(lhs_transposed.Rank() -
                                                      1);
  ...
  std::vector<Axis> rhs_contracting_dimensions_values(rhs_transposed.Rank() -
                                                      1);
