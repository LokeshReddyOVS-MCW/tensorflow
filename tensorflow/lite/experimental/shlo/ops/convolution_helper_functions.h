...
void SliceImpl(const Tensor& operand, int64_t num_outputs, size_t start_indices,
               size_t inner_dimensions_size, size_t outer_dimensions_size,
               int64_t dimension, Tensor& output) {
...
template <DataType storage_type>
void PaddingImpl(ConvolutionOp& op, const Tensor& operand) {
  using StorageT = StorageType<storage_type>;
...
// DotGeneral function implementation
template <DataType storage_type>
absl::Status DotGeneralImpl(ConvolutionOp& op, const Tensor& lhs,
  ...
  for (size_t k = 0; k < output_size; ++k, ++output_data) {
    ...
    for (size_t i = 0; i < op.lhs_result_dimensions.size(); ++i, ++result_dim) {
      lhs_index[op.lhs_result_dimensions[i]] = output_index[result_dim];
    }
    for (size_t i = 0; i < op.rhs_result_dimensions.size(); ++i, ++result_dim) {
      if (result_dim < output.Rank()) {
        rhs_index[op.rhs_result_dimensions[i]] = output_index[result_dim];
      }
    }
    output_element = 0;
    while (true) {
