...
absl::Status PrepareImpl(ConvolutionOp& op, const Tensor& lhs,
  ...
  // DotGeneral prepare
  DimVector<DimensionSize> dims(rhs.Rank());
  ...
  std::vector<Axis> lhs_contracting_dimensions_values(lhs.Rank() - 1);
  ...
  std::vector<Axis> rhs_contracting_dimensions_values(rhs.Rank() - 1);
